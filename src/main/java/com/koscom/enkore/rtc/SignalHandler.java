package com.koscom.enkore.rtc;

import com.fasterxml.jackson.databind.ObjectMapper;;
import com.koscom.enkore.domain.Room;
import com.koscom.enkore.domain.RoomMap;
import com.koscom.enkore.dto.WebSocketMessage;
import com.koscom.enkore.service.RoomService;
...
public class SignalHandler extends TextWebSocketHandler {
    ...
    /**
     *   WebRTC 참고
     *   1. https://github.com/Benkoff/WebRTC-SS/blob/master/src/main/java/io/github/benkoff/webrtcss/socket/SignalHandler.java
     *   2. https://github.com/SeJonJ/Spring-WebSocket-WebRTC-Chatting/blob/master-Webrtc-jpa/src/main/java/webChat/rtc/SignalHandler.java
    ...
    private Map<String, Room> sessionIdToRoomMap = RoomMap.getInstance().getSessionIdToRoomMap();
    ...
    public void afterConnectionClosed(WebSocketSession session, CloseStatus status) {
        ...
        Room rm = sessionIdToRoomMap.get("3");
        Map<String, WebSocketSession> clients = roomService.getClients(rm);

        for(String key : clients.keySet()) {
            WebSocketSession a = clients.get(key);
            if(a.equals(session)){
                clients.remove(key);
                break;
            }
        ...
        try {
            ...
            switch (message.getType()) {
                ...
                case MSG_TYPE_ICE:
                    ...
                    /* 여기도 마찬가지 */
                    Room rm = sessionIdToRoomMap.get(roomId);
                ...
                case MSG_TYPE_JOIN:
                    ...
                    logger.info("[ws] {} has joined Room: #{}", userUUID, message.getData());

                    //room = roomService.findRoomByStringId(roomId)
                    //        .orElseThrow(() -> new IOException("Invalid room number received!"));
                    room = sessionIdToRoomMap.get(roomId);

                    // room 안에 있는 userList 에 유저 추가
                    roomService.addClient(room, userUUID, session);

                    // 채팅방 입장 후 유저 카운트+1
                    // chatServiceMain.plusUserCnt(roomId);

                    sessionIdToRoomMap.put(roomId, room);
                    break;
                ...
                case MSG_TYPE_LEAVE:
                    ...
                    client.ifPresent(userID -> roomService.removeClientByName(room, userID));

                    // 채팅방에서 떠날 시 유저 카운트 -1
                    // chatServiceMain.minusUserCnt(roomId);

                    logger.info("삭제 완료 [{}] ",client);
                    break;
